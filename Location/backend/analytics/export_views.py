...
def export_rapport_mensuel_excel(request):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        resume_data = {
            ...
            'Valeur': [
                float(paiements.aggregate(total=Sum('montant'))['total'] or 0),
                float(depenses.aggregate(total=Sum('montant'))['total'] or 0),
                float(paiements.aggregate(total=Sum('montant'))['total'] or 0) -
                float(depenses.aggregate(total=Sum('montant'))['total'] or 0),
                paiements.count(),
                depenses.count(),
...
def export_rapport_annuel_excel(request):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for mois in range(1, 13):
            ...
            total_revenus = paiements_mois.aggregate(total=Sum('montant'))['total'] or 0
        ...
        for bien in Bien.objects.all():
            ...
            biens_data.append({
                ...
                'Type': bien.type_bien,
                'Statut': bien.get_statut_display(),
                'Revenus annuels': float(paiements_bien.aggregate(total=Sum('montant'))['total'] or 0),
        ...
        for client in Client.objects.all():
            ...
            if paiements_client.exists():
                clients_data.append({
                    ...
                    'Total payé': float(paiements_client.aggregate(total=Sum('montant'))['total'] or 0),
